...
from plottext.infrastructure.service.yh_service import download_real_data, get_nasdaq100_data_async, filter_nasdaq100_data
...
async def keep_purged_stocks_screen():
    ...
    """
    console.clear()
    keep_purge_dict = config()["keep_purge"]
    keep_df, purge_df = dfs_from_dict(keep_purge_dict) # change by file or rest api..
...
async def run_strategy_of_day_screen():
    ...
    with console.status("[bold green]Descargando datos..."):
        data_nasdaq100 = await get_nasdaq100_data_async(period="1y", interval="1d", force_download=(force_download == "y"), max_age_days=1)
    ...
    keep_df = pd.DataFrame(keep_purge_dict["keep"]) # change by file or rest api..
